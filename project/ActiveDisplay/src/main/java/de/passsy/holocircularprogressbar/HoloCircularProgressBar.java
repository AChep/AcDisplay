 ...
 * <p><i>Modified by Artem Chepurnoy [artemchep@gmail.com]</i></p>
 *
...
public class HoloCircularProgressBar extends View {
	...
	public HoloCircularProgressBar(final Context context, final AttributeSet attrs) {
		this(context, attrs, com.achep.activedisplay.R.attr.circularProgressBarStyle);
	}
	...
	public HoloCircularProgressBar(final Context context, final AttributeSet attrs, final int defStyle) {
		...
		final TypedArray attributes = context.obtainStyledAttributes(attrs, com.achep.activedisplay.R.styleable.HoloCircularProgressBar,
				defStyle, 0);

		setProgressColor(attributes.getColor(com.achep.activedisplay.R.styleable.HoloCircularProgressBar_progress_color, Color.CYAN));
		setProgressBackgroundColor(attributes.getColor(com.achep.activedisplay.R.styleable.HoloCircularProgressBar_progress_background_color,
				Color.MAGENTA));
		setProgress(attributes.getFloat(com.achep.activedisplay.R.styleable.HoloCircularProgressBar_progress, 0.0f));
		setMarkerProgress(attributes.getFloat(com.achep.activedisplay.R.styleable.HoloCircularProgressBar_marker_progress, 0.0f));
		setWheelSize((int) attributes.getDimension(com.achep.activedisplay.R.styleable.HoloCircularProgressBar_stroke_width, 10));
		mIsThumbEnabled = attributes.getBoolean(com.achep.activedisplay.R.styleable.HoloCircularProgressBar_thumb_visible, true);
		mIsMarkerEnabled = attributes.getBoolean(com.achep.activedisplay.R.styleable.HoloCircularProgressBar_marker_visible, true);

		mGravity = attributes.getInt(com.achep.activedisplay.R.styleable.HoloCircularProgressBar_android_gravity, Gravity.CENTER);
    ...
    public void setProgress(final float progress, boolean animation) {
    ...
    private class R implements Runnable {
        ...
        public void run() {
            ...
            if (now > end) {
                ...
                setProgress((float) (end - now) / duration * progress, true);
                mHandler.postDelayed(mRunnable, 16);
            }
        }

        public void prepare(float progress, long duration) {
            this.progress = progress;
            this.duration = duration;
            this.end = SystemClock.uptimeMillis() + duration;
        }
    ...
    public void animateProgressFrom(float progress, long duration) {
        mRunnable.prepare(progress, duration);
        mHandler.post(mRunnable);
    }

    public void animateProgressFromOne(long duration) {
        animateProgressFrom(1f, duration);
    }

    public void cancelAnimateProgress() {
        mRunnable.cancel();
    }
